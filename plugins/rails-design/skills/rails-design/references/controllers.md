...
- 認可gem（Pundit / CanCanCan）は入れない。スコープと `can_*?` 述語で足りる
...
トランザクション・イベント記録・関連レコードの後始末はすべてモデル側に置く。
コントローラが知っているのは「誰の要求か」「何を呼ぶか」「どう返すか」の3つだけ。

1行に詰め込むのは狙わない。`@post.publish && redirect_to(@post)` は、
...
- **書き込みは bang**（`create!` / `update!` / `destroy!`）。
  バリデーション失敗をUIで扱う経路だけ `if` で分岐する。
  「常にbang」ではなく「**失敗をユーザーに見せない経路はbang**」
- **Rails 8以降は `params.expect`**。キーが無い／型が違うときに `ParameterMissing` を返す。
  `wrap_parameters` をクラス冒頭に置いてHTMLフォームとJSON APIの入力形を揃える
- **非同期は `_later` サフィックス**。ジョブクラスは1行にしてモデルのメソッドを呼ぶだけ。
